...
| **Sharing** | Granting other registered GeoGami users (by email) access to one of your games' tracks, managed from the dashboard sidebar. Shared games appear in the recipients' dashboards; the creator can revoke access at any time. Sharing works on **drafts** too — a game can be shared and played before it is published. |
| **Co-author** (*editor*) | A registered user the game owner adds (by email) to collaborate on a game. Co-authors can **edit** and **publish** the game and **see its tracks**, but cannot delete it or manage the co-author list. Shared games appear in the co-author's **My games** with a *Shared* marker. Distinct from **Sharing**, which grants track access only. |
| **Draft** | A newly created game, visible only to its creator (in *My games*, marked with a draft badge) and to `contentAdmin`/`admin` (in the *Drafts* tab). Drafts are kept out of the public game list, so test and unfinished games don't clutter it. |
| **Published** | A game the creator has made publicly available via the *Publish* action; it then appears in the public *Published* list for everyone. Publishing is self-service — there is no separate admin curation step (this replaced the old "curated games" concept). |
